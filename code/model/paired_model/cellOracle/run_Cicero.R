...
save(conns_cicero_hg38, file = "cicero_PBMC_peak_gene.rda")

# Prepare peak list for cellOracle input
all_peaks <- row.names(exprs(input_cds))
id3 <- match(all_peaks, peaks_hg19)

# Check for unmatched peaks
length(which(is.na(id3)))

# Map peaks to hg38 and replace ":" with "_"
all_peaks1 <- peaks_hg38[id3]
all_peaks1 <- gsub(":", "_", all_peaks1)

# Save peak list and Cicero connections as CSV for cellOracle
write.csv(all_peaks1, file = "cellOracle_PBMC_all_peaks.csv")
write.csv(conns_cicero_hg38, file = "cellOracle_PBMC_cicero_connections.csv")
