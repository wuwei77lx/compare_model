...
def main():
    ...
    # compute the average
    cols_for_avg = list(filter(lambda x: "hic_contact" in x, all_hic.columns))
    ...
    all_hic.to_csv(
        os.path.join(args.outDir, args.chromosome, args.chromosome + ".bed.gz"),
        sep="\t",
        header=False,
        index=False,
        compression="gzip",
...
def scale_hic_with_powerlaw(hic, resolution, scale_ref, gamma_ref, scale, gamma):
    ...
    hic["hic_contact"] = hic["hic_contact"] * (pl_ref / pl)
...
def process_chr(
    ...
    hic_file, hic_norm_file, is_vc = get_hic_file(
        chromosome,
        os.path.join(basedir, cell_type),
        allow_vc=True,
    )
    if is_vc:
        return None
    ...
    pl_summary = pd.read_csv(
        os.path.join(
            basedir, cell_type, "powerlaw/hic.powerlaw.tsv"
        ),
        sep="\t",
    )
    ...
    hic = load_hic_juicebox(
        ...
        window=np.Inf,
        min_window=0,
        gamma=-1 * pl_summary["hic_gamma"].values[0],
    ...
    hic = scale_hic_with_powerlaw(
        ...
        scale_ref,
        gamma_ref,
        scale=pl_summary["hic_scale"].values[0],
        gamma=pl_summary["hic_gamma"].values[0],
    )

    # fill nan in hic matrix with special value
    # this will be turned back to nan after merging
    assert not np.any(hic["hic_contact"].values == special_value)
    hic.loc[np.isnan(hic["hic_contact"]), "hic_contact"] = special_value
